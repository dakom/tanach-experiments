...
```sh
tanach-calc print-gematria-verse --book 19 --chapter 1 --verse 1
```
...
```sh
tanach-calc print-gematria-sequence --start-position 1 --length 12
```
...
`"913, 203"` searches for the targets `[913, 203]`.

By default the search **accumulates**: each `--sequence` value is a *running-sum
target*, and a contiguous run of letters (or whole words) that *sums* to it is a
match. This is the usual gematria question — the word *bereishit* is 913, so
`--sequence 913` finds every place whose letters add up to 913. Multiple values
match **consecutive adjacent runs**: `--sequence 913,203` finds a run summing to
913 immediately followed by one summing to 203. Since letter values are positive,
each starting point yields at most one match. Pass `--no-accumulate` (below) to
match each value against a single letter exactly instead.

Two modes:

- `--mode verse` — matches within a single verse, and each match must **start on
  a word boundary**: accumulation adds up **whole words** from the start of each
  word toward the end of that verse (crossing words but not the verse end), so a
  match spans a whole number of words. Hits report `(book, chapter, verse, word)`.
- `--mode letters` — matches anywhere in the flat letter stream, starting at any
  letter: accumulation adds up **letter by letter** and may cross word, verse, and
  book boundaries (and end mid-word). Hits report the exact start
  `(book, chapter, verse, word, letter)`.

Add `--reverse` to read from the **last letter of the Tanach toward the first**
(accumulating from the last letter/word backward). Each hit's reported location is
its start *in reading order* (the highest-position letter); in `verse` mode this
anchors on each word's **last** letter (the mirror of forward's first-letter
anchor). Backward hits are listed from the end of the Tanach backward; `matches
per book` stays in canonical order either way.

```sh
tanach-calc search-gematria --sequence 913 --mode verse
```
...
- The command builds an in-memory index from the per-book files (which supply the
  chapter/verse mapping) in one pass — see [`src/search.rs`](src/search.rs) — so
  no extra precomputed data file is needed. Every search runs in a single pass
  (`O(total letters + matches)`).

#### Exact matching (`--no-accumulate`)

With `--no-accumulate`, each `--sequence` value must match **one letter exactly**
(its gematria value), and the whole sequence is found as a literal run of
consecutive letter values via KMP. So `"2, 30,5,10, 12"` searches for the five
adjacent letters `[2, 30, 5, 10, 12]`. In `verse` mode the run still must start on
a word boundary; in `letters` mode it may start anywhere. Exact hits omit
`length`/`end_position` (the span always equals the query length).

```sh
tanach-calc search-gematria --sequence "2,200,1,300,10,400" --mode verse --no-accumulate
```
...
```sh
tanach-calc search-gematria --sequence 5 --mode letters --start 51 --limit 50
```

```
search (mode: letters, accumulate) targets: 5
matches: 108366
matches per book:
  ... (complete, all 39 books)
showing hits 51-100 of 108366 (start 51, limit 50)
hits:
  ... (50 hits)
```
...
```json
"pagination": { "start": 51, "limit": 50, "total_hits": 108366,
